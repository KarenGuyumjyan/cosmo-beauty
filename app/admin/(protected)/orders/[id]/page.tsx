...
export default async function OrderDetailPage({ params }: Props) {
  ...
  return (
    <div className="p-8 max-w-3xl">
      <div className="mb-8">
        <a href="/admin/orders" className="text-sm text-stone-400 hover:text-rose-600 transition-colors">
          ← Back to Orders
        </a>
      ...
      <div className="bg-white rounded-2xl border border-stone-100 p-6 mb-5">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div><p className="text-stone-400 text-xs mb-0.5">Name</p><p className="font-medium text-stone-800">{order.customerName}</p></div>
          <div><p className="text-stone-400 text-xs mb-0.5">Phone</p><p className="font-medium text-stone-800">{order.customerPhone}</p></div>
          {order.customerEmail && (
            <div><p className="text-stone-400 text-xs mb-0.5">Email</p><p className="font-medium text-stone-800">{order.customerEmail}</p></div>
          )}
          <div><p className="text-stone-400 text-xs mb-0.5">Date</p><p className="font-medium text-stone-800">{new Date(order.createdAt).toLocaleString()}</p></div>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-stone-50">
            {order.items.map((item) => (
              <tr key={item.id}>
                <td className="px-6 py-4 font-medium text-stone-800">{item.product.nameEn}</td>
                <td className="px-6 py-4 text-stone-500">{item.quantity}</td>
                <td className="px-6 py-4 text-stone-500">{item.price.toLocaleString()} ₽</td>
          ...
          <tfoot>
            <tr className="border-t border-stone-200">
              ...
              <td className="px-6 py-4 text-right font-bold text-rose-600 text-base">{order.total.toLocaleString()} ₽</td>
            </tr>
          </tfoot>
        </table>
      </div>
